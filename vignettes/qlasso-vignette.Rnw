...
lrats = fitShape(t(log2(counts+1)))$lrats
(pval <- round(shapeManova(lrats, groups)$pval, 3))
          
plot(lrats, col=groupCol, pch=19, ylab="L-kurt", xlab="L-skew",
     main="Pickrell (Summary of raw log2(counts+1) values)",
     sub=paste0("Wilk's Manova Test: pval=", pval))
@
